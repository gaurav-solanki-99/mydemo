import 'package:flutter/material.dart';
import 'package:get/get.dart';

import 'package:get/get.dart';

import 'package:http/http.dart' as http;

import '../Model/UserModel.dart';
import '../NetWorksApi/userapi.dart';

class ProductController extends GetxController{

  RxList<Datum> rxProducts2 = <Datum>[].obs;

  var noImage ="data:image/jpeg;base64,".obs;







  loadData(BuildContext context)
  {

    getUserApi(context,).then((value) {

      if(value!="")
      {

         // userlist=usersFromJson(value).data;


          rxProducts2.assignAll(usersFromJson(value).data);

          print("rxProducts2 >>>>>>"+rxProducts2.length.toString());
      }



    });
  }
  @override
  void onInit() {

    super.onInit();
  }




}